...
public class TopologicalSortAdjacencyList {
    ...
    // Single Source Shortest Path problem can be efficiently solved on DAG using
    // this method in O(V+E)
    // NP-hard on DAG can be solved using this method: Trick is to multiply all edge
    // weight by -1 and then find the shortest path and multiply the weights by -1 again.
    //
    //
    // NOTE: 'numNodes' is not necessarily the number of nodes currently present
    // in the adjacency list since you can have singleton nodes with no edges which
